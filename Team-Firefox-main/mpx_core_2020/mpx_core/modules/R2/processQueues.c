...
queue suspendedBlockedQueue = {0,NULL,NULL};
queue suspenededReadyQueue = {0,NULL,NULL};
...
void addToSuspendedBlockedQueue(pcb* PCB){
	queue* q = getSuspendedBlockedQueue();
	q->count++;
	//it is the only entry
	if(q->count == 1){
		q->head = PCB;
