...
pcb * createPCB(char name[], int pcbClass,int priority,char status[]){
	...
	if((findPCB(name) == NULL && strlen(name)>=8 ) && (pcbClass == 0 || pcbClass == 1) && (priority>=0 && priority <10)&& strcmp(status,"Not Suspended")==0){
		pcb* temp = setupPCB(name,pcbClass,priority,status);
		addToReadyQueue(temp);
		return temp;
	}
